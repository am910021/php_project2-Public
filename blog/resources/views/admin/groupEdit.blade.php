...
<div class="container-fluid">
  <div class="row">
    <div class="col-md-12">
      <h3 class="text-center text-info" >正在編輯群組： {{ $group->name }} </h3>
      <br>
    </div>
  </div>
  <div class="row">
      ...
      <form class="form-horizontal" action="{{ route('admin.groupUpdate', ['id'=>$group->id]) }}" method="post">
        ...
        <div class="form-group {{ $errors->has('group_manager')?"has-error":"" }}">
          <div class="col-md-offset-2 col-md-5 col-sm-7">
            <input type="hidden" class="form-control" id="group_manager" name="group_manager" value="{{ $group->manager()->id }}" >
            <div class="input-group">
              <span class="input-group-addon">&nbsp;&nbsp;管&nbsp;理&nbsp;員&nbsp;</span>
              <input type="text" class="form-control" id="group_text" aria-describedby="managerHelp" value="{{ $group->manager()->username }}" readonly onclick="selectGroup()" >
            </div>
